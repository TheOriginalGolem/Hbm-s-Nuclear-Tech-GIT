...
public class ExplosionBalefire {
    ...
    private void breakColumn(int x, int z) {
        ...
        if (dist > 0) {
            ...
            int startDepth = 6 - 6 * dist / radius;
            for (int i = 0; i <= startDepth; i++) {
                if (worldObj.getBlockState(new BlockPos(pX, depth - i, pZ)).getBlock() == Blocks.STONE) {
                    switch (startDepth - i) {
                        case 6:
                            worldObj.setBlockState(new BlockPos(pX, depth - i, pZ), ModBlocks.sellafield_core.getDefaultState());
                            break;
                        case 5:
                            worldObj.setBlockState(new BlockPos(pX, depth - i, pZ), ModBlocks.sellafield_4.getDefaultState());
                            break;
                        case 4:
                            worldObj.setBlockState(new BlockPos(pX, depth - i, pZ), ModBlocks.sellafield_3.getDefaultState());
                            break;
                        case 3:
                            worldObj.setBlockState(new BlockPos(pX, depth - i, pZ), ModBlocks.sellafield_2.getDefaultState());
                            break;
                        case 2:
                            worldObj.setBlockState(new BlockPos(pX, depth - i, pZ), ModBlocks.sellafield_1.getDefaultState());
                            break;
                        case 1:
                            worldObj.setBlockState(new BlockPos(pX, depth - i, pZ), ModBlocks.sellafield_0.getDefaultState());
                            break;
                        case 0:
